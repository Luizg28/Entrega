...
public class TelaConsultaPeca extends JInternalFrame {
    
        private JTextField tfDescricaoPeca; 

	public TelaConsultaPeca(String titulo) {
		super(titulo);
		...
		btnPesquisar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				Query query = manager.createQuery("select f from Peca as f " + "where f.descricaoPeca = :parametro1");
				query.setParameter("parametro1", tfDescricaoPeca.getText());

				List<Peca> lista = query.getResultList();

				for (Peca peca : lista) {
					JOptionPane.showMessageDialog(null, "Nome da Peca: " 
                                            + peca.getDescricaoPeca());
				}
		...
		groupLayout.setVerticalGroup(groupLayout.createParallelGroup(GroupLayout.Alignment.LEADING)
				.addGroup(groupLayout.createSequentialGroup().addGap(50)
						...
						.addGap(26)));
		groupLayout.linkSize(SwingConstants.VERTICAL, new Component[] { lNome, tfDescricaoPeca });
